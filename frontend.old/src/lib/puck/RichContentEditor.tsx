...
/**
 * RichContentEditor — TipTap WYSIWYG + raw HTML toggle
...
export function RichContentEditor({ value, onChange }: RichContentEditorProps) {
  ...
  // Sync external value changes into editor (e.g. when Puck resets)
  useEffect(() => {
    if (!editor) return;
    ...
    <div className="rounded-lg border border-zinc-200 bg-white overflow-hidden">
      ...
      {rawMode ? (
        <textarea
          ...
          className="w-full bg-zinc-50 p-3 font-mono text-xs text-zinc-700 outline-none resize-none border-0"
          rows={10}
          spellCheck={false}
        />
      ) : (
        <EditorContent
          ...
          className="
            tiptap-editor
            min-h-[180px] cursor-text p-5 text-[15px] text-zinc-800 outline-none
