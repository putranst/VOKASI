...
// SFIA Level Labels
const SFIA_LEVELS: Record<number, string> = {
    1: 'Follow',
    2: 'Assist',
    3: 'Apply',
    4: 'Enable',
...
// Helper to check if agenda has enterprise features
const isEnterpriseAgenda = (agenda: any): boolean => {
    return agenda?.program_learning_outcomes && agenda?.sfia_competency_map;
};
            ...
            <div className="space-y-6">
                ...
                {teachingAgenda && (
                    <div className="space-y-6">
                        ...
                        {/* Enterprise Curriculum Features (MIT OCW Standard) */}
                        {isEnterpriseAgenda(teachingAgenda) && (
                                ...
                                {(teachingAgenda as any).assessment_strategy?.summative && (
                                    <div className="p-5 bg-white border border-gray-200 rounded-2xl">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                            {Object.entries((teachingAgenda as any).assessment_strategy.summative).map(([key, value]: [string, any]) => (
                                                <div key={key} className="p-3 bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl text-center">
                                                    <div className="text-2xl font-black text-green-700">{value}%</div>
                                                    <div className="text-xs text-green-600 capitalize">{key.replace('_', ' ')}</div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
