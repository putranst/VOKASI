...
export default function AdminCoursesPage() {
  ...
  if (loading) return <div className="text-slate-400 p-8">Loading courses...</div>;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">{courses.length} courses across all institutions</p>
      </div>
          ...
          <tbody className="divide-y divide-slate-800">
            {f.map(c => (
              <tr key={c.id} className="hover:bg-slate-800/50 transition-colors">
                ...
                <td className="px-5 py-4 text-slate-300">{c.instructor}</td>
                <td className="px-5 py-4 text-slate-400">{c.institution}</td>
                <td className="px-5 py-4"><span className={`px-2 py-1 rounded text-xs font-medium ${SC[c.status]}`}>{c.status}</span></td>
                <td className="px-5 py-4 text-slate-300">{c.enrollments}</td>
