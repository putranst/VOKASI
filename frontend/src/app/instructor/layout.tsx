...
const NAV_ITEMS = [
  ...
  { href: "/instructor/documents", label: "Documents", icon: "📄" },
];
...
export default function InstructorLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!token || (role !== "instructor" && role !== "admin")) router.push("/login");
  }, [router]);
      ...
      <div className="flex">
        <aside className="w-56 shrink-0 bg-slate-900 border-r border-slate-800 min-h-screen sticky top-0 hidden lg:flex flex-col">
          <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-800">
            ...
            <span className="font-bold text-blue-400 tracking-wide">INSTRUCTOR</span>
          </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          <header className="flex items-center justify-end px-6 py-3 border-b border-slate-800 bg-slate-900/50 sticky top-0 z-30">
            <div className="flex items-center gap-4">
              ...
              <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400 font-bold text-sm">I</div>
            </div>
          </header>
